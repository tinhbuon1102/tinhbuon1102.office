								...
								<section class="profile-avatar mb-none">
									<figure class="profile-image">
										...
										<a style="display: none;" image-type="Logo" data-toggle="modal" data-target="#popover_content_wrapper" class="picture-upload-trigger ng-hide" id="profileImageUploader" editbtn-valign="top" ng-class="{'btn-small': editbtnSize == 'small'}" ng-style="editBtnStyle" ng-show="profile.mode === 'edit'" title="プロフィール写真を編集"> <span class="picture-upload-trigger-inner"> <svg version="1.1" id="Layer_1" xmlns:sketch="http://www.bohemiancoding.com/sketch/ns" xmlns="http://www.w3.org/2000/svg"
											...
											</svg> <span class="picture-upload-trigger-text" style="display: none;">プロフィール写真を編集</span>
										</span>
									...
									<div class="basic-user-info-wrapper">
										<ul class="basic-user-info">
											<li class="locate"><span class="fa fa-map-marker awesome-icon">{{$user->Prefecture}}{{$user->City}}</span>
											</li>
											<li class="sex">
												<!--if male--> <span class="fa {{$user->Sex == "男性" ? 'fa-mars' : 'fa-venus'}} awesome-icon">{{$user->Sex}}</span> <!--if female--<span class="fa fa-female awesome-icon">女性</span>-->

											</li>
											<li class="age"><span class="fa fa-user awesome-icon">{{getUserAage($user)}}歳</span>
											</li>
										</ul>
									</div>
									<div class="profile-verified">
										<ul class="verified-list">
											<li class="<?php echo $isPaymentSetup ? 'is-verified' : ''?> verified-item verified-payment"><span class="Icon"> <i class="fa fa-credit-card" aria-hidden="true"></i>
											</span>
											</li>
											<li class="<?php echo $isProfileFullFilled ? 'is-verified' : ''?> verified-item verified-indentify"><span class="Icon"> <i class="fa fa fa-user" aria-hidden="true"></i>
											</span>
											</li>
											<li class="<?php echo $user->Tel ? 'is-verified' : ''?> verified-item verified-phone"><span class="Icon"> <i class="fa fa fa-phone" aria-hidden="true"></i>
											</span>
											</li>
											<li class="<?php echo $user->IsEmailVerified == 'Yes' ? 'is-verified' : ''?> verified-item verified-email"><span class="Icon"> <i class="fa fa fa-envelope" aria-hidden="true"></i>
											</span>
											</li>
										</ul>
									</div>
										...
										<div id="profile-about-description-wrapper" class="profile-about-description-wrapper">
											<div class="editable-block-wraper">
												<div class="job profile-title-block editable-block">
													<span class="edit-content"> <!--label--> <span class="job-label">職種：</span> <!--/label--> <span class="job-title">{{$user->BusinessType}}</span>
													</span>
												</div>
                        <!--
                    ...
                    -->
                      ...
                      <select data-placeholder="スキルを選択" class="chosen-select" id="profile-skills"  multiple="multiple" aria-hidden="true" tabindex="-1">
								...
								<section class="profile-statistics mb-none">
									<div class="top-pad-inner">
										...
										<div class="profile-btn">
											<div class="offer-btn">
												<a class="btn btn-large profile-btn edit-profile-btn"> <span class="fa fa-pencil awesome-icon">プロフィール編集</span>
												</a> <a class="btn btn-large profile-btn view-profile-btn" style="display: none;"> <span class="fa fa-empty awesome-icon">プロフィール表示</span>
												</a>
											</div>
										</div>
										<?php }?>
									...
									<ul class="item-stats" ng-show="profile.user.role === 'freelancer'">
										<li class="is-good"><span class="item-stats-name">使用清潔度</span> <span class="item-stats-value">{{$reviews['CleaninessAvg']}}%</span>
										</li>
										<li class="is-good"><span class="item-stats-name">礼儀正しさ</span> <span class="item-stats-value">{{$reviews['PoliteAvg']}}%</span>
										</li>
										<li class="is-good"><span class="item-stats-name">ルール遵守</span> <span class="item-stats-value">{{$reviews['KindnessAvg']}}%</span>
										</li>
										<li class="is-good"><span class="item-stats-name">再利用希望</span> <span class="item-stats-value">{{$reviews['RepeatAvg']}}%</span>
										</li>
									</ul>
									<?php if(Auth::guard('user1')->check()) {?>
                                    <div class="send-msg">
									<a href="/ShareUser/Dashboard/Message/{{$user->HashCode}}">
										<button class="btn button msg-button yellow-button" data-bind="click: SendMessageViewModel.showDialog" role="button" aria-disabled="false"><span class="ui-button-text"><i class="icon-offispo-icon-06 awesome-icon"></i>メッセージを送る</span></button>
									</a>
                                    </div>
                                    <?php }?>
								</section>
                                ...
                                <div class="profile-intro-username edit-widget is-editable ng-isolate-scope">
									...
									<span class="edit-content"> <!--label--> <span class="job-label">職種：</span> <!--/label--> <span class="job-title">{{$user->BusinessType}}</span></span>
									</div>
                                ...
                                @if( empty($reviews['CleaninessAvg']) ) 
                                ---%
                                @else
                                {{$reviews['CleaninessAvg']}}%
                                @endif
                                </div>
                                <div class="profiles-user-statistics-item ng-binding">
                                <span class="profiles-user-statistics-title">ルール遵守</span>
                                
                                @if( empty($reviews['KindnessAvg']) ) 
                                ---%
                                @else
                                {{$reviews['KindnessAvg']}}%
                                @endif
        ...
        $('#popover_content_wrapper').on('show.bs.modal', function (e) {
			...
			// Init avatar image
	     	<?php /*if (isset($_SESSION['avatar_image'])) :?>
	     		...
	     		$('.modal.in #filename').val('<?php echo $_SESSION['avatar_image']?>');
	 		<?php endif*/ ?>
   		})
...
jQuery.ajaxSetup({
	...
	jQuery(document).ready(function($){
		...
		jQuery('#btnSave').click(function(){
			jQuery(this).parent('div').siblings('span').children('a').click();
			var pref=jQuery('#prefecture option:selected').text();
			var dist=jQuery('#district').val().join(',');
			var town=jQuery("#town option:selected").map(function() {return this.value;}).get().join(',');
			...
			var usageFrequency = jQuery('#choose_frequency').val();
			var businessType = jQuery('#BusinessType_workplace').val();
			var workPlaceNumberOfPeople = jQuery('#desire_numpeople_inoffice').val();
			var budgetType = jQuery('#choose_budget_per').val();
			var budgetId = jQuery('#choose_budget_per_').val();
			var notes = jQuery('#textnotes').val();
			var spacetype = jQuery('.SpaceType:checked').map(function() {return this.value;}).get().join(',');
			var meeting = jQuery('input[name="meetingroom"]:checked').val();


			 jQuery.ajax({
            type: "POST",
            url : '/RentUser/Dashboard/MyProfile/Edit',
            data : { SpaceType:spacetype,DesireLocationPrefecture:pref,DesireLocationDistricts:dist,DesireLocationTown:town,
					BudgetType:budgetType,BudgetID:budgetId,TimeSlot:timeslot,SpaceArea:spaceArea,
					NumberOfPeople:numpeople,MeetingRoom:meeting,UsageFrequency:usageFrequency,
					BusinessType:businessType,WorkPlaceNumberOfPeople:workPlaceNumberOfPeople},
           success: function(data){
		   }
		   });
			updateDetail();

			jQuery('#spanlocation').text(pref+','+dist+','+town);

		});
		jQuery('#btnCancel').click(function(){
			jQuery(this).parent('div').siblings('span').children('a').click();
		});
		jQuery('#btnSaveEqp').click(function(){
			jQuery(this).parent('div').siblings('span').children('a').click();
            ...
            type: "POST",
            url : '/RentUser/Dashboard/MyProfile/Edit',
            data : { numOfDesk:numDesk,NumOfChair:numChair,NumOfBoard:numBorad,NumOfLargeDesk:numLargeDesk,OtherFacility:otFacility},
			success: function(data){
			   }
			});

			jQuery('#spnDesk').text(numDesk);
			jQuery('#spnChair').text(numChair);
			jQuery('#spnBoard').text(numBorad);
			jQuery('#spnLargeDesk').text(numLargeDesk);
			jQuery('#spnOtherFacility').text(otFacility);
		});
		jQuery('#btnCancelEqp').click(function(){
			jQuery(this).parent('div').siblings('span').children('a').click();
		});
	...
	{
		setTimeout(function(){jQuery('#prefecture').val('<?php echo $space->DesireLocationPrefecture ?>');jQuery('#prefecture').change();},1000);
		setTimeout(function(){jQuery('#district').val('<?php echo $space->DesireLocationDistricts ?>'.split(','));jQuery('#district').change();},3000);
		/*setTimeout(function(){
			var v = jQuery('.chosen-results').text();
			console.log(v);
			jQuery('#district').find("option").first().remove(); 
			jQuery('.chosen-results').find("li").first().remove(); 
		},10000);*/
		setTimeout(function(){
		jQuery('#town').val('<?php echo $space->DesireLocationTown ?>'.split(',')).trigger('chosen:updated');
		},5000);
		jQuery('#choose_budget_per').val('<?php echo $space->BudgetType ?>');
		jQuery('#choose_budget_per').click();
		...
		jQuery('#choose_frequency').val('<?php echo $space->UsageFrequency ?>');

		jQuery('#BusinessType_workplace').val('<?php echo $space->BusinessType ?>');

		jQuery('#desire_numpeople_inoffice').val('<?php echo $space->WorkPlaceNumberOfPeople ?>');

		setTimeout(function(){jQuery('#choose_budget_per_').val('<?php echo $space->BudgetID ?>');},200);
		var mtRoom = '<?php echo $space->MeetingRoom ?>'.toLowerCase();
		jQuery('input[id="'+mtRoom+'_meetingroom"]').attr('checked','checked');
		...
		jQuery('#spnDesk').text('<?php echo $space->NumOfDesk ?>');
		jQuery('#spnChair').text('<?php echo $space->NumOfChair ?>');
		jQuery('#spnBoard').text('<?php echo $space->NumOfBoard ?>');
		jQuery('#spnLargeDesk').text('<?php echo $space->NumOfLargeDesk ?>');
		jQuery('#spnOtherFacility').text('<?php echo $space->OtherFacility ?>');
	...
	{
		jQuery('#spntimeslot').text(jQuery('#choose_timeslot :selected').text());
		jQuery('#spnnumpeople').text(jQuery('#choose_numpeople :selected').text());
		jQuery('#spnSpaceArea').text(jQuery('#choose_spacearea :selected').text());
		jQuery('#spnUsageFrequency').text(jQuery('#choose_frequency :selected').text());
		jQuery('#spnBusinessType').text(jQuery('#BusinessType_workplace :selected').text());
		jQuery('#spnWorkPlaceNumberOfPeople').text(jQuery('#desire_numpeople_inoffice :selected').text());
		setTimeout(function(){jQuery('#budgetType').text(jQuery('#choose_budget_per_ :selected').text());},300);
		jQuery('#spannotes').text(jQuery('#textnotes').val());
		jQuery('#spancspacetype').text(jQuery('.SpaceType:checked').map(function() {return this.value;}).get().join(','));
			jQuery('#spanmeeting').text(jQuery('input[name="meetingroom"]:checked').val());
