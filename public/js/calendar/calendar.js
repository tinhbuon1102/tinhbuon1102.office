							...
							button = $( // type="button" so that it doesn't submit a form
								...
								.click(function(ev) {
									...
									if (!button.hasClass(tm + '-state-disabled')) {
										...
										button.trigger('afterButtonClick');
...
var MonthGrid = DayGrid.extend({
	...
	renderDayRowHtml: function(row, isRigid) {
		...
		'<div class="' + classes.join(' ') + '">' +
			'<div class="fc-bg">' +
				'<table>' +
					this.renderNumberTrHtml(row, 'cell') +
				'</table>' +
			'</div>' +
			'<div class="fc-content-skeleton">' +
				'<table>' +
	...
	renderNumberTrHtml: function(row, isCell) {
		return '' +
			'<tr>' +
				this.renderNumberCellsHtml(row, isCell) +
			'</tr>';
	},
	...
	renderNumberCellsHtml: function(row, isCell) {
		var htmls = [];
		var col, date;
		...
		for (col = 0; col < this.colCnt; col++) {
			...
			if (isCell)
				...
				htmls.push(this.renderNumberMonthHtml(date));
		}
	...
	renderNumberMonthHtml: function(date) {
		var classes;
	...
	updateDayTable: function() {
		...
		this.dayDates = dates;
		this.dayIndices = offsets;
		this.daysPerRow = this.view.opt('monthsPerRow');
		this.colCnt = this.daysPerRow;
	...
	renderSegPopoverContent: function(row, col, segs) {
		var view = this.view;
		var isTheme = view.opt('theme');
		var title = this.getCellDate(row, col).format(view.opt('monthPopoverFormat'));
		var content = $(
			'<div class="fc-header ' + view.widgetHeaderClass + '">' +
				'<span class="fc-close ' +
					(isTheme ? 'ui-icon ui-icon-closethick' : 'fc-icon fc-icon-x') +
...
var YearView = FC.YearView = BasicView.extend({
	...
	scroller: null,
	dayGrid: null, // the main year-grid subcomponent of this view
	...
	computeRange: function(date) {
		var range = BasicView.prototype.computeRange.call(this, date); // get value from super-method
		range.start = range.intervalStart;
		var rowCnt;
	...
	renderDates: function() {
		...
		this.el.addClass('fc-year-view').html(this.renderSkeletonHtml());
		this.headRowEl = $(); // This element is not in the year view
		
		this.scroller.render();
		var dayGridContainerEl = this.scroller.el.addClass('fc-month-grid-container');
		var dayGridEl = $('<div class="fc-month-grid" />').appendTo(dayGridContainerEl);
		this.el.find('.fc-body > tr > td').append(dayGridContainerEl);
...
fcViews.year = {
		'class': YearView,
		duration: { years: 1 }, // important for prev/next
		defaults: {
			monthsPerRow: 3,
