	...
	public function sales(Request $request)
	{
		...
		if ($request->tab == 'total_sales')
		{
			...
			$rent_datas=Rentbookingsave::select(DB::raw('
					...
					SUM(if('.$purchasedCondition.', amount_user1_sale - ChargeFee + refund_amount, 0)) as total_gross_sale,
					SUM(if('.$purchasedCondition.', (ChargeFee * 2) + refund_amount, 0)) as total_net_sale ,
					AVG(if('.$purchasedCondition.', amount_user1_sale - ChargeFee + refund_amount, 0)) as avarage_gross_sale,
			...
			$rent_datas=Rentbookingsave::select(DB::raw('*, COUNT(*) as count_booking, SUM(amount_user1_sale) as total_amount, SUM(ChargeFee) as total_charge_fee'))
