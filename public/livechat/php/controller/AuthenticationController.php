    ...
    public function isGuestLoggedInAction()
    {
        ...
        if (!empty($user) && !$user['id'])
        {
        	...
        	$user = new UserModel(array(
        			...
        			'name'          => $user['name'] . '- User',
        			'mail'          => $user['mail'],
        			'password'      => 'thisisrealuser',
        			'roles'         => array('GUEST'),
        			'last_activity' => $user['last_activity'],
        			'image' 		=> $user['image'] ? $user['image'] : '/livechat/upload/default-avatars/a.png',
        	));
        	$options = array();
        	$options['name'] = $user->name;
        	$options['mail'] = $user->mail;
        	$options['password'] = 'thisisrealuser';
        	 
        	$exists = UserModel::repo()->findOneBy($options);
        	if(!$exists)
        	{
        		$user->save();
        	}
        	else {
        		$user = UserModel::repo()->findOneBy($options);
        	}
