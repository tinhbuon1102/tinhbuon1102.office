                            ...
                            <div class="dataTable_wrapper">
							  ...
							  <table class="table table-striped table-bordered table-hover" id="dataTables-example">
									...
									@foreach($users as $user)
                                        <tr class="odd gradeX">
                                            <td><input tabindex="1" type="checkbox" name="delete[]" id="delete-{{$user->id}}" value="{{$user->id}}"> </td>
                                            <td>{{$user->NameOfCompany}}</td>
                                            <td>{{$user->LastName}} {{$user->FirstName}}</td>
                                            <td>{{$user->Email}}</td>
                                            <td>@if($user->IsAdminApproved=="Yes")
													承認済み
												@else
													未承認
												@endif
                                            ...
                                            <td><a href="User1/View/{{$user->id}}">View</a>
												@if($user->IsAdminApproved=="No") 
													 | <a href="User1/Approve/{{$user->id}}">承認する</a>
												@endif
											</td>
                                        </tr>
									@endforeach	
