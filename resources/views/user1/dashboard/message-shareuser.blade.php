						...
						<div class="chat-left">
							...
							<ul class="chat-list">
								@foreach($chatusers as $cuser)
									<li>
									<img src="@if(!empty($cuser->user2->Logo)){{$cuser->user2->Logo}}@elseif($cuser->user2->Sex=='女性'){{'/images/woman-avatar.png'}}@else{{'/images/man-avatar.png'}}@endif" class="chat-pic" /><div class="chat-count" >7</div>
									<h2>{{getUserName($cuser->user2)}}</h2><div class="chat-st"></div>
									<p>@if($cuser->latestChat()!=null)
										{{str_limit($cuser->latestChat()->Message,35)}}
									@endif &nbsp;
									</p><div class="chat-time">{{$cuser->latestChat()->created_at->diffForHumans()}}</div>
								</li>
								@endforeach
								<li>
