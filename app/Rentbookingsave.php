...
class Rentbookingsave extends Model
{
	...
	{
		return $this->hasMany('App\BookingRecursion','BookingID','id')->whereNotNull('status')->groupBy('NextScheduled')->orderBy('id','desc');
	}
	...
	public function storeRecursionHistory($rent_datas = array()) {
		if (!count($rent_datas))
		{
			$rent_datas = \App\Rentbookingsave::where('recur_id', '<>', '')
				->whereIn('status', array(BOOKING_STATUS_REFUNDED))
			...
			foreach ($rent_datas as $rent_data)
			{
				try{
					if($rent_data->payment_method == 'creditcard'){
						...
						$existRecursion = \App\BookingRecursion::where('BookingID', $rent_data->id)
						->where('RecursionID', $rent_data->recur_id)
						->first();
						
						$sTimeNow = strtotime(date('Y-m-d H:i:s'));
						if (!$existRecursion || ($existRecursion->NextScheduled && $sTimeNow > $existRecursion->NextScheduled))
						{
							$info = $webpay->recursion->retrieve(array('id' => $rent_data->recur_id));
					...
					elseif($rent_data->payment_method == 'paypal'){
						...
						if (!$existRecursion || ($existRecursion->NextScheduled && $sTimeNow > $existRecursion->NextScheduled))
						{
							...
							if ($info && $info['ACK'] == PAYPAL_RESPONSE_STATUS_SUCCESS)
							{
								...
								$oRecursion->InitialPayment = !$existRecursion ? 1 : 0;
								$oRecursion->Deleted = in_array($info['STATUS'], array('Cancelled', 'Suspended', 'Expired')) ? 1: 0;
								$success = $oRecursion->save();
							}
						}
					}
				...
				{
					return $this->processException(array('error' => 1, 'function' => 'storeRecursionHistory', 'response' => $e));;
				}
			}
		}
		return true;
	...
	public function reSaveStartEndBookingTime()
	{
		...
		foreach ($rent_datas as $rent_data)
		{
			...
			if (count($slots_data))
			{
				...
				if (in_array($rent_data->SpaceType, array(SPACE_FEE_TYPE_MONTHLY, SPACE_FEE_TYPE_WEEKLY)))
				{
	...
	function testApi()
	{
		try{
			...
			// 			$info = $webpay->charge->refund(['id' => 'ch_3u8g4S5Dy2KO1jZ', 'amount' => '664']);
		}catch (\Exception $e)
		{
			return $this->processException(array('error' => 1, 'function' => 'testAPI', 'response' => $e));
		}
		pr($before);
		die;
	}
	...
	public function deleteRecursion($rent_data)
	{
		...
		if ($rent_data->recur_id)
		{
			if($rent_data->payment_method == 'creditcard'){
				try{
					$webpay = new WebPay(WEPAY_SECRET_API_KEY);
					$webpay->recursion->delete(array('id' => $rent_data->recur_id));
	...
	private function cancelBookingPayment(&$rent_data, $refund_status = '')
	{
		...
		if (in_array($rent_data->status, array(BOOKING_STATUS_PENDING, BOOKING_STATUS_DRAFT)))
		{
			$refund_status = BOOKING_REFUND_NO_CHARGE;
		}
		
		// Return refund status when need get information such as click refund button
		if (isset($rent_data->return_info) && $rent_data->return_info)
		{
			return array('refund_status' => $refund_status);
		}
		...
		if ($rent_data->payment_method == 'creditcard')
		{
			...
			}catch(\Exception $e)
			{
				...
				return $this->processException(array('error' => 1, 'function' => 'cancelBookingPayment', 'response' => $e));
			}
		}
		if ($rent_data->payment_method == 'paypal')
		{
			try {
				...
				if ($refund_status != BOOKING_REFUND_CHARGE_100)
				{
					...
					{
						$REFUNDTYPE = 'Full';
						$refundData['RTFields'] = array('TRANSACTIONID' => $rent_data->transaction_id, 'REFUNDTYPE' => $REFUNDTYPE, 'CURRENCYCODE' => 'JPY');
					}
					
// 					$getTransData['GTDFields'] = array('TRANSACTIONID' => $rent_data->transaction_id);
// 					$response = $PayPal->GetTransactionDetails($getTransData);

					$response = $PayPal->RefundTransaction($refundData);
					
					$voidData['DVFields'] = array('AUTHORIZATIONID' => $rent_data->transaction_id);
	...
	private function captureBookingPayment(&$rent_data)
	{
		if ($rent_data->payment_method == 'creditcard')
		{
			...
			try {
				...
				$info = $webpay->charge->capture(array('id' => $rent_data->transaction_id));
				if ($info->captured)
				{
			...
			}catch(\Exception $e)
			{
				...
				return $this->processException(array('error' => 1, 'function' => 'captureBookingPayment', 'response' => $e));
			}
		}
		...
		if ($rent_data->payment_method == 'paypal')
		{
			try {
				...
				return $this->processException(array('error' => 1, 'function' => 'processCancelPayment', 'response' => $e));
			}
		}
	}
	...
	public function processCancelPayment($rent_data, $isAuto = true)
	{
		...
		try {
			...
			{
				#- PEDING should be changed to CANCELLED if start date has passed as pending
				return $this->cancelBookingPayment($rent_data, BOOKING_REFUND_NO_CHARGE);
			}
				
			elseif (!$isAuto) {
				// If cancelled by USER
				$aChargeStatus = array(BOOKING_STATUS_DRAFT, BOOKING_STATUS_PENDING, BOOKING_STATUS_RESERVED);

				if (!in_array($rent_data->status, $aChargeStatus))
				{
					return array('error' => 1, 'function' => 'processCancelPayment', 'response' => trans('common.Your booking status is not able to refund !'));
				}
				...
				switch ($rent_data->SpaceType)
				{
					case SPACE_FEE_TYPE_HOURLY :
						 ...
						 */
						$aDiff = ['free' => 2, 'half_charge' => 1, 'full_charge' => 1];
					...
					case SPACE_FEE_TYPE_DAYLY:
						...
						$aDiff = ['free' => 7, 'half_charge' => 3, 'full_charge' => 3];
						break;
					...
					case SPACE_FEE_TYPE_WEEKLY:
						...
						$aDiff = ['free' => 14, 'half_charge' => 7, 'full_charge' => 7];
					...
					case SPACE_FEE_TYPE_MONTHLY :
						...
						$aDiff = ['free' => abs($oNextMonth->diffInDays($oTimeNow)), 'half_charge' => 21, 'full_charge' => 21];
						break;
				}
	...
	public function processPaidPayment($rent_data, $isAuto = true)
	{
		...
		if ($isAuto)
		{
			...
			{
				#- RESERVED should be changed to IN-USE while start time starts until end time
		...
		{
			return $this->processException(array('error' => 1, 'function' => 'processPaidPayment', 'response' => $e));
		}
	}
	...
	*/
	public function processBookingPaymentAuto($rent_datas = array()) {
		...
		$processStatus = array(BOOKING_STATUS_PENDING, BOOKING_STATUS_RESERVED);
		$sTimeNow = date(DATE_TIME_DEFAULT_FORMAT);
		if (!count($rent_datas))
		{
			$rent_datas = self::whereIn('status', $processStatus)->where('charge_start_date', '<=', $sTimeNow)->take(20)->get();
		}
				...
				if ($slot->Type == 'HourSpace')
				{
					...
					if (isCoreWorkingOrOpenDesk($rent_data->spaceID))
					{
						if ($oParentSlot)
						{
							...
							$oaBookedSlots = \App\Spaceslot::select('total_booked')->where('ParentID', $slot->ParentID)->where('Type', 'HourSpace')->get();
							$total_booked = 0;
	...
	public function storeBookingHistory($rent_data, $storedType)
	{
		...
		{
			if (!in_array($attField, array('id', 'created_at', 'updated_at')))
			{
	...
	public function bookingSaveCallBack($rent_data, $storedType = 'status')
	{
		...
		$aHostMembers = [$rent_data->shareUser->Email];
		...
		if ($storedType == 'status')
		{
			switch ($rent_data->status)
			{
				...
				case BOOKING_STATUS_PENDING :
					...
					{
						try{
							$this->storeRecursionHistory(array($rent_data));
						}
						catch(\Exception $e)
						{
	...
	public function getBookedSlots($booking, $isArchive = true) {
		if ($isArchive)
			$oSlot = new \App\Bookedspaceslot();
		else
			$oSlot = new \App\Spaceslot();

		$slots = array();
		if ($isArchive)
		{
			$slots = $oSlot->where('BookedID', $booking->id)->orderBy('StartDate', 'ASC')->orderBy('StartTime', 'ASC')->get();
		}
		else {
			$bookedSlotIds = explode(';', $booking->spaceslots_id);
			$slots = $oSlot->whereIn('id', $bookedSlotIds)->orderBy('StartDate', 'ASC')->orderBy('StartTime', 'ASC')->get();
		}
	...
	public function getFinalCancelAttribute()
	{
		...
		if(date('Y-m-d',strtotime($finalcancel)) < date('Y-m-d',strtotime($this->created_at))){
			//$rentDate_= Carbon::createFromFormat(DATE_TIME_DEFAULT_FORMAT,$this->spaceSlot->StartDate.' '.$this->spaceSlot->StartTime);
			//$finalcancel= $rentDate_->subHours(1);

			$created_at = $this->spaceSlot->created_at;
			$finalcancel= $created_at->addHours(1);
			//echo "<pre>rent_data:"; print_r($this->spaceSlot->created_at); echo "</pre>";
			//echo "<pre>rent_data2:"; print_r($this->created_at); echo "</pre>";
		}
		return $finalcancel;
	}
}
